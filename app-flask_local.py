...
# Check if request contains a file (CSV upload)
    if 'file' in request.files:  # CSV Upload Case
        ...
        if file.filename.endswith('.csv'):
            ...
            for row in reader:
                if 'UserId' not in row or 'Name' not in row:
                    return jsonify({'error': 'UserId and Name are required in CSV'}), 400
                users.append({
                    'UserId': row['UserId'],
                    'Name': row['Name'],
            ...
            # Batch insert into DynamoDB (if needed)
            with table.batch_writer() as batch:
                for user in users:
                    batch.put_item(Item=user)

            return jsonify({'message': 'Users added successfully', 'users': users}), 201
    ...
    if isinstance(data, list):  # Multiple users
        users = []
        for user in data:
            if 'UserId' not in user or 'Name' not in user:
                return jsonify({'error': 'UserId and Name are required'}), 400
            users.append({
                'UserId': user['UserId'],
                'Name': user['Name'],
